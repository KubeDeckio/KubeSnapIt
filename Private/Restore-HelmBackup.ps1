function Restore-HelmBackup {
    ...
    # Locate corresponding history and backend files based on timestamp
    $backupDir = [System.IO.Path]::GetDirectoryName($ManifestFilePath)
    $historyFile = Get-ChildItem -Path $backupDir -Filter "${releaseName}_history_${timestamp}.yaml" | Select-Object -First 1
    $backendFile = Get-ChildItem -Path $backupDir -Filter "${releaseName}_backend_${timestamp}.json" | Select-Object -First 1

    # Log the files being used for restoration
    Write-Host "Using manifest file: $ManifestFilePath" -ForegroundColor Green
    if ($historyFile) {
        Write-Host "Using history file: $($historyFile.FullName)" -ForegroundColor Green
    } else {
        Write-Host "No matching history file found for release '$releaseName'." -ForegroundColor Yellow
    }
    if ($backendFile) {
    ...
    if (-not $DryRun) {
        $namespaceCheck = kubectl get namespace $Namespace -o name -ErrorAction SilentlyContinue
    ...
    Write-Host "Restoring manifest for Helm release '$releaseName'..."
    if ($DryRun) {
    ...
    if ($backendFile) {
        Write-Host "Restoring backend storage for Helm release '$releaseName'..."
        if ($DryRun) {
    ...
    if ($historyFile) {
        Write-Host "Restoring history for Helm release '$releaseName'..."
        if ($DryRun) {
            Write-Host "Dry run: Would restore history from file $($historyFile.FullName)." -ForegroundColor Yellow
        } else {
            $kubectlApplyCmd = "apply -f $($historyFile.FullName)"
            Write-Verbose "Running command: kubectl $kubectlApplyCmd"
            kubectl apply -f $historyFile.FullName
            Write-Host "History for release '$releaseName' restored to backend storage." -ForegroundColor Green
        }
    }
